...
$this->title = 'ใบแจ้งหนี้: ' . $model->code;
...
<div class="invoice-view">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-tools">
                ...
                <?= Html::a('<i class="fas fa-money-bill-wave"></i> บันทึกการรับชำระ', ['receipts/create', 'invoice_id' => $model->id], ['class' => 'btn btn-success']) ?>
                    ...
                    <?= DetailView::widget([
                        ...
                        'attributes' => [
                            ...
                            [
                                ...
                                'format' => 'raw',
                                'value' => $model->quotation ? 
                                    Html::a($model->quotation->code, ['quotations/view', 'id' => $model->quotation->id], ['class' => 'text-primary']) 
                                    : '-',
                            ],
                            [
                ...
                <table class="table table-bordered">
                    ...
                    <tfoot>
                        ...
                        <tr class="table-warning">
                            <td colspan="5" class="text-right"><strong>ชำระแล้ว:</strong></td>
                            <td class="text-right"><strong><?= number_format($model->paid_amount, 2) ?></strong></td>
                        </tr>
                        <tr class="table-info">
                            <td colspan="5" class="text-right"><strong>คงเหลือ:</strong></td>
                            <td class="text-right"><strong><?= number_format($model->balance, 2) ?></strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    <div class="card mt-3">
        <div class="card-header">
            <h4><i class="fas fa-money-bill-wave"></i> ประวัติการรับชำระ</h4>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= Html::a($receipt->code, ['receipts/view', 'id' => $receipt->id], ['class' => 'text-primary']) ?></td>
                            <td><?= Yii::$app->formatter->asDatetime($receipt->received_at) ?></td>
                            <td class="text-right"><?= number_format($receipt->amount, 2) ?></td>
                            <td><?= $receipt->method == 1 ? 'เงินสด' : ($receipt->method == 2 ? 'โอน' : 'เช็ค') ?></td>
